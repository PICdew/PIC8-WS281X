...
//use weak pull-ups so open input pins don't float:
    NOT_WPUEN = FALSE; //use WPUA
//    WPUA = PORTA_MASK & ~0;
//    IFWPUBC(WPUBC = PORTBC_MASK & ~0);
//set output pins, leave non-output pins as hi-Z:
    TRISA &= ~(Abits(ROWSOUT_MASK) | Abits(SEROUT_MASK)); //0b111111; //Abits(COL_PINS) | Abits(pin2bits16(SERIN);
    WPUA = WREG;
//NOTE: SPEN overrides TRIS
    TRISBC &= ~(BCbits(ROWSOUT_MASK) | BCbits(SEROUT_MASK)); //0b010000; //BCbits(COL_PINS) | BCbits(pin2bits16(SERIN);
    IFWPUBC(WPUBC = WREG);
//    PORTC = 0xff;
    PORTA = PORTBC = 0;
...
//    wait(15 msec); //needs a little time between commands
    wait(25 msec); //needs a little time between commands
}
#ifdef mp3_send
 #undef mp3_send
#endif
