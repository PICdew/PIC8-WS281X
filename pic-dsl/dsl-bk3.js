...
//            this.consts = {};
        const symtab = {};
        const ast = !opts.reduce? reduce(ast_raw, symtab): ast_raw;
...
//            console.error(`${Object.keys(this.funcs).length} funcs: ${Object.keys(this.funcs).map((key) =>
        Object.keys(symtab).forEach((key) => { if (!symtab[key]) warn(`undefined symbol in '${nameof(ast_raw)}': '${key}'`); }); //check for undefined stuff within this context
        console.error(`${Object.keys(symtab).length} symbols: ${Object.keys(symtab).map((key) =>
        {
            return `${symtype(symtab[key])}:${key}`; //|| "(UNDEF)"}`;
        }).join(", ")}`);
//        if (opts.traverse) opts.traverse(ast.body, opts); //strip off wrapper before passing to custom DSL processor
        traverse(ast, {}, (evt, data) => { retval.emit(evt, data); });
//            else this.push("TODO: code gen\n");
//            compiled.execode
        cb();
    }
...
//        context = context || this.symtab; //default to globals
    ...
    switch (ast_node.type)
    {
        case "CallExpression": //{type, callee{}, arguments[]}
            if (ignore(nameof(ast_node.callee))) return NULL_EXPR; //exclude Node.js run-time functions
...
//                const type = (ast_node.kind == "const")? "consts": "vars";
        ...
        case "ArrowFunctionExpression": //{type, id{}, params[], defaults[], body{}, generator, expression}
            ...
            (ast_node.params || []).forEach((param, inx, all) => { all[inx] = reduce(param, locals); locals[nameof(param)] = param2var(all[inx]); }); //treat as defined vars
...
/*
//generate a call graph from AST:
function callgraph(ast_node, tree)
{
}
*/

//traverse AST, send nodes downstream:
function traverse(ast_node, symtab, emitter)
{
    if (!ast_node) return;
    switch (ast_node.type)
    {
        case "CallExpression": //{type, callee{}, arguments[]}
            (ast_node.arguments || []).forEach((arg, inx, all) => { traverse(arg, symtab, emitter); });
//            symtab[nameof(ast_node.callee)] = symtab[nameof(ast_node.callee)] || null; //create fwd ref if not defined yet
            break;
        case "BinaryExpression": //{type, operator, left{}, right{}}
        case "LogicalExpression": //{type, operator, left{}, right{}}
        case "AssignmentExpression": //{type, operator, left{}, right{}}
            traverse(ast_node.left, symtab, emitter);
            traverse(ast_node.right, symtab, emitter);
//                console.error(`coalese lhs ${ast_node.left.type} ${ast_node.operator} rhs ${ast_node.right.type}? ${isconst(ast_node.left) && isconst(ast_node.right)}`);
            break;
        case "BlockStatement": //{type, body[]}
            (ast_node.body || []).forEach((stmt, inx, all) => { traverse(stmt, symtab, emitter); });
            break;
        case "ExpressionStatement": //{type, expression{}}
            traverse(ast_node.expression, symtab, emitter);
            break;
        case "VariableDeclarator": //{type, id, init{}, kind-inherited}
            traverse(ast_node.init, symtab, emitter);
//            if (symtab[nameof(ast_node)]) throw `duplicate ${ast_node.kind || "var?"} def: ${nameof(ast_node)}`.red_lt;
//            symtab[nameof(ast_node)] = (ast_node.kind == "const")? (isconst(ast_node.init)? makeconst(ast_node.init.value): ast_node.init): ast_node; //"TODO: eval?";
            break;
        case "Identifier": //{type, name}
//            symtab[nameof(ast_node)] = symtab[nameof(ast_node)] || null; //create fwd ref if not defined yet
        ...
        case "ArrowFunctionExpression": //{type, id{}, params[], defaults[], body{}, generator, expression}
            const locals = Object.assign({}, symtab); //shallow copy to new stack frame
            (ast_node.params || []).forEach((param, inx, all) => { traverse(param, locals, emitter); }); //treat as defined vars
            (ast_node.defaults || []).forEach((def, inx, all) => { traverse(def, locals, emitter); });
            traverse(ast_node.body, locals, emitter);
//                console.error(`upon ${nameof(ast_node)} exit, local symbols are: ${Object.keys(locals).map((key) => { return `${symtype(locals[key])}:${key}`; })}`);
//                console.error(`upon ${nameof(ast_node)} exit, parent symbols are: ${Object.keys(symtab).map((key) => { return `${symtype(symtab[key])}:${key}`; })}`);
//            console.error(`${Object.keys(locals).length - Object.keys(symtab).length} symbols defined/used in '${nameof(ast_node)}': ${Object.keys(locals).traverse((all_mine, key) => { if (!(key in symtab)) all_mine.push(`${symtype(locals[key])}:${key}`); return all_mine; }, []).join(", ")}`.cyan_lt); //NOTE: mine undef === null, sibling undef === false
//no, could be siblings, defined after                Object.keys(locals).forEach((key) => { if (!locals[key]) warn(`undefined symbol in '${nameof(ast_node)}': '${key}'`); }); //check for undefined stuff within this context
//            if (symtab[nameof(ast_node)]) throw `duplicate func def: ${nameof(ast_node)}`.red_lt;
//            symtab[nameof(ast_node)] = isconst(ast_node.body)? makeconst(ast_node.body.value): ast_node; //"TODO: func val?"; //update parent context
            break;
        case "IfStatement": //{type, test{}, consequent{}, alternate{}}
            traverse(ast_node.test, symtab, emitter);
            traverse(ast_node.consequent, symtab, emitter);
            traverse(ast_node.alternate, symtab, emitter);
            break;
        case "ForStatement": //{type, init{}, test{}, update{}, body{}}
            traverse(ast_node.init, symtab, emitter);
            traverse(ast_node.test, symtab, emitter);
            traverse(ast_node.update, symtab, emitter);
            traverse(ast_node.body, symtab, emitter);
            break;
        case "WhileStatement": //{type, test{}, body{}}
            traverse(ast_node.test, symtab, emitter);
            traverse(ast_node.body, symtab, emitter);
        ...
        case "UnaryExpression": //{type, operator, argument{}}
        case "UpdateExpression": //{type, operator, argument{}, prefix}
            traverse(ast_node.argument, symtab, emitter);
            break;
        case "YieldExpression": //{type, argument{}, delegate}
            traverse(ast_node.argument, symtab, emitter);
            break;
//////////////
        case "VariableDeclaration": //{type, declarations[], kind}
            (ast_node.declarations || []).forEach((dcl, inx, all) => { traverse(dcl, symtab, emitter); });
            break;
        case "ArrayExpression": //{type, elements[]}
            (ast_node.elements || []).forEach((expr, inx, all) => { traverse(expr, symtab, emitter); });
            break;
        case "ThrowStatement": //{type, argument{}}
        case "ReturnStatement": //{type, argument{}}
        case "ThisExpression": //{type}
        ...
        case "NewExpression": //{type, callee{}, arguments[]}
            (ast_node.arguments || []).forEach((arg, inx, all) => { traverse(arg, symtab, emitter); });
            traverse(ast_node.callee, symtab, emitter);
            break;
        case "SwitchStatement": //{type, discriminant{}, cases[]}
            traverse(ast_node.discriminant, symtab, emitter);
            (ast_node.cases || []).forEach((casestmt, inx, all) => { traverse(casestmt, symtab, emitter); });
            break;
        case "SwitchCase": //{type, test{}, consequent[]}
            traverse(ast_node.test, symtab, emitter);
            (ast_node.consequent || []).forEach((conseq, inx, all) => { traverse(conseq, symtab, emitter); });
            break;
        case "TemplateLiteral": //{type, quasis[], expressions[]}
            (ast_node.quasis || []).forEach((quasi, inx, all) => { traverse(quasi, symtab, emitter); });
            (ast_node.expressions || []).forEach((expr, inx, all) => { traverse(expr, symtab, emitter); });
            break;
        case "TemplateElement": //{type, value{}, tail}
            traverse(ast_node.value, symtab, emitter);
            break;
        case "MemberExpression": //{type, computed, object{}, property{}}
            traverse(ast_node.object, symtab, emitter);
            traverse(ast_node.property, symtab, emitter);
            break;
        case "BreakStatement": //{type, label}
        case "EmptyStatement": //{type}
        case "Literal" : //{type, value, raw}
            break;
        default: //for debug
            throw `DSL traverse: unhandled node type '${ast_node.type}', node ${JSON5.stringify(ast_node, null, "  ")}`.red_lt;
    }
...
//            var callee = (ast_node.callee.type == "MemberExpression")? `${ast_node.callee.object.name}.${ast_node.callee.property.name}`: ast_node.callee.name;
            ...
            (ast_node.arguments || []).forEach((arg, inx, all) => { traverse(arg, `arg[${inx}/${all.length}]`, nest + LEVEL); });
//                if (!THIS.seen[nameof(ast_node.callee)]) THIS.seen[nameof(ast_node.callee)] = null; //leave placeholder for AST; traverse later; //toAST(ast_node.callee); //{ THIS.funclist.push(nameof(ast_node.callee)); }
            traverse(ast_node.callee, "callee", nest + LEVEL);
            break;
        case "ForStatement": //{type, init{}, test{}, update{}, body{}}
...
//                console.error(`${nest}'${name}': throw stmt`)
            evt.children = ["argument"];
            traverse(ast_node.argument, "throw stmt", nest + LEVEL);
            break;
        case "IfStatement": //{type, test{}, consequent{}, alternate{}}
...
{
    return {type: "VariableDeclarator", id: param_node, init: null, kind: "var"};
}
...
{
    const Types = {U: "undef", F: "func", L: "lit", V: "var"};
    return Types[((ast_node || {}).type || "U").slice(0, 1)] || "UNKN";
}
